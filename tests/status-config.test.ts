...
describe('status command', () => {
  it('shows placeholders when project data is missing', async () => {
    ...
    expect(output).toContain('[not yet generated]');
    expect(output).toContain('mnemo-hook');
  });
...
describe('config command', () => {
  ...
  it('prints the stored config table', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls.map((call) => String(call[0])).join('\n');
    expect(output).toContain('language [change]');
    expect(output).toContain('landing_page_url [change] [remove]');
    expect(output).toContain('created_at');
    expect(output).toContain('updated_at');
  });

  it('rejects invalid language values', async () => {
    await expect(runConfigCommand(adapter, { language: 'jp' }, 'owner--repo')).rejects.toThrow('Invalid --language value "jp". Valid values: en, ko, both.');
  });
});
