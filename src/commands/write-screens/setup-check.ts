...
import { deriveRepoSlug, getConfig } from '../../resources/project-config.js';
import { analyzeProject } from '../../resources/project-metadata.js';
...
async function showFirstRunPrompt(info: Awaited<ReturnType<typeof analyzeProject>>): Promise<boolean> {
  console.log(`  Language : ${info?.detected_language ?? 'unknown'}`);
...
export async function runSetupCheck(storage: StorageAdapter): Promise<{ repo_slug: string; skip: boolean }> {
  const repo_slug = await resolveRepoSlug();
